...
public class RedGen : MonoBehaviour
{
    ...
    float span = 2.5f; //2.5f마다 적십자를 생성한다.
    ...
    void Update()
    {
        ...
        if (this.delta > this.span)//델타값이 span시간보다 많거나 같다면 다음의 작업을 수행한다.
        {
            ...
            GameObject go = Instantiate(RedPrefab) as GameObject;//RedPrefab을 게임 오브젝트로 내보낸다.
            int px = Random.Range(-6, 7);//적십자의 x좌표는 -6부터6까지 랜덤으로 위치하라
